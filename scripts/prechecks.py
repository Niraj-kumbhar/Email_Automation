...
class prechecks:
    ...
    def __init__(self,i=0):
        try:
            if i==1: # passing arg to avoid auto run
                ...
                Slog.write("==*10"+f"\n\n{datetime.now()}-->Started Prechecks\n")
    ...
    def data(self):
        try:
            ...
            Slog.write(f"\n{datetime.now()}-->reading email_data file")
            ...
            Slog.write(f"\n{datetime.now()}-->reading email_data file completed")
            self.error_check()
        
        except Exception as e:
            print("Error in prechecks - data reading")
            Slog.write(f"\n{datetime.now()}--> Error Occured {e}")
    ...
    def error_check(self):
        try:
            Slog =  open('..//logs/Scriptlogs.txt','a') # logs tracking
            Slog.write(f"\n{datetime.now()}--> checking for errors..")
            ...
            Slog.write(f"\n{datetime.now()}--> checking for errors completed")
